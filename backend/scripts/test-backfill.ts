...
function test(name: string, passed: boolean, details = '') {
...
async function runTests() {
  ...
  try {
    const { initQueues, getQueues, closeQueues, getQueueStats } = await import('../src/services/queue');
  ...
  try {
    const { generateSyntheticHistoricalData, getEventsNeedingEnrichment } = await import('../src/services/historicalFetcher');
    
    // Generate 1 week of test data (dry run)
    const endDate = new Date();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - 7);
    
    const { events } = await generateSyntheticHistoricalData(startDate, endDate, { dryRun: true });
    ...
    testEndDate.setDate(testEndDate.getDate() - 1000); // Far in the past to not interfere
  ...
  try {
    const { initQueues, addEventImportJob, getQueueStats, closeQueues, clearAllQueues } = await import('../src/services/queue');
  ...
  // Test 5: End-to-end mini backfill (import only)
  console.log('\n🔄 Testing Mini Backfill (Import Only)...');
  let workersStarted = false;
  try {
    const { initQueues, addEventImportJob, closeQueues, clearAllQueues } = await import('../src/services/queue');
    const { startWorkers, stopWorkers, getWorkerStats, resetWorkerStats } = await import('../src/services/backfillWorkers');
    ...
    // Add import job for 3 days of data (far in the past)
    const testEnd = new Date();
    testEnd.setDate(testEnd.getDate() - 500);
    ...
    while (attempts < 15) {
      ...
      console.log(`    Waiting... imported=${workerStats.eventsImported}`);
    ...
    if (workersStarted) {
      try {
        const { stopWorkers } = await import('../src/services/backfillWorkers');
        const { closeQueues } = await import('../src/services/queue');
        await stopWorkers();
        await closeQueues();
      } catch (e) {}
    }
